...
def predict(classifier_path, output_path, compare, info, use_cuda, Cband,
		Sband, Xband, no_plot, classifierThreshold=0.9,
	...
	if use_large_classifier:
		classifier = models.ClassifierLarge(band_list = givenBands, in_channels=len(givenBands), out_channels=1)
	else:
		classifier = models.ClassifierSmall(band_list = givenBands, in_channels=len(givenBands), out_channels=1)
	...
	with torch.no_grad():
		classifierOutput = torch.sigmoid(classifier(input)).squeeze(0)
